...
GamePlatform::GamePlatform(int iSize, int iType, int iXStart, int iXEnd, int iYStart, int iYEnd, float fXPos, float fYPos, bool direction, int seesawPlatformID) {
	this->iSize = iSize;
	this->iType = iType;
...
GamePlatform::~GamePlatform(void) {
...
void GamePlatform::Draw(SDL_Renderer* rR) {
...
void GamePlatform::moveYReverse() {
	switch(iType) {
		case 6:
			fYPos -= 2;
			iXStart = (int)fYPos;
...
int GamePlatform::getXPos() {
	return (int)fXPos;
}

int GamePlatform::getYPos() {
	return (int)fYPos;
}

int GamePlatform::getSize() {
	return iSize;
}

void GamePlatform::setTypeID(int iType) {
	this->iType = iType;
}

int GamePlatform::getTypeID() {
	return iType;
}

void GamePlatform::turnON() {
	this->ON = true;
}